...
export default function Home() {
  ...
  const handleDeploy = async () => {
    ...
    try {
      ...
      // Wait for a minimum of 30 seconds before proceeding
      const timerPromise = new Promise(resolve => setTimeout(resolve, 30000));
      
      // Wait for both deployment and timer to complete
  ...
  const getMainContentHeight = () => {
    if (!isClient) return '100vh'; // Default for SSR
    // Subtract header height (h-16 is 4rem which is 64px)
